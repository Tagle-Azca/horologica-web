...
export function EditorialSection({ label, text, pullQuote }: EditorialSectionProps) {
  return (
    ...
    >
      ...
      {pullQuote && (
        <p className="font-serif text-2xl text-neutral-200 italic leading-snug mb-8 border-l-2 pl-6" style={{ borderColor: 'var(--color-gold-400)' }}>
          {pullQuote}
        </p>
      )}

      <p className="font-sans text-sm text-neutral-400 leading-loose">{text}</p>
    </motion.section>
  );
}
